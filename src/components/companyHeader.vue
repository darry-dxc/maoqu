    ...
    <el-menu
      ...
      <el-row style="position: absolute; top:0;width:100%;margin: 0 auto;padding-right: 30px">
        ...
        <el-col :span="4" :offset="6" style="height: 57px;padding-top: 12px;color: white;text-align: right;text-overflow: ellipsis;padding-right: 12px">
          <span>{{nickCompanyName}}</span><br/>
          <span>{{nickCompanyMan}}</span><br/>
        </el-col>
...
export default {
    ...
    mounted(){
       let data11=JSON.parse(localStorage.getItem('Name2'));
      this.$store.commit("updateCompanyName",data11);
       let data111=JSON.parse(localStorage.getItem('Name22'));
      this.$store.commit("updateCompanyMan",data111);
    },
    computed:{
       nickCompanyMan(){
    ...
    methods:{
      ...
      viewInfo(){
        this.$router.push({ path: "/WebCompany/EnterpriseCenter/EnterDocumentManage" })
      }
  }
};
</script>
