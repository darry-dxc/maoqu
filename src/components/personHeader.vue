  ...
  <div >
    <el-menu
      ...
      <el-row style="position: absolute; top:0;width:100%;margin: 0 auto;padding-right: 30px">
        ...
        <el-col :span="4" :offset="6" style="height: 57px;line-height: 57px;color: white;text-align: right;text-overflow: ellipsis;padding-right: 12px">
          <span>{{nickName}}</span>
        </el-col>
...
export default {
    ...
    mounted(){
      let data1=JSON.parse(localStorage.getItem('Name1'));
      this.$store.commit("updateName",data1);
    },
    computed:{
       nickName(){
    ...
    methods:{
      ...
      handleCommand(command) {
        if(command === "a"){
          this.$router.push({ path: "/AccountSafe1" })
        }else {
          this.logout()
        }
      },
      viewInfo(){
        this.$router.push({ path: "/PersonDocumentManage" })
      }
  }
};
</script>
