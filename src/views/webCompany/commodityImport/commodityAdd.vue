          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="企业商品编号" :label-width="formLabelWidth" class="form-item" :required="isAdd">
                <el-input v-model="form.enterpriseCommodityNo" size="small" @blur="isCommodityExist" v-if="isAdd"/>
                <el-autocomplete
                  size="small"
                  v-if="!isAdd"
    ...
    export default {
      ...
      methods:{
        ...
        getCommodityIds(query,cb){
          this.$axios3.post('import/getCommodityIds', {
            enterpriseCommodityNo:this.form.enterpriseCommodityNo
          }).then((res) => {
        ...
        selectCommodity(item){
          this.$axios3.post('import/getCommodityById',{
            commodityId:item.commodityId
          }).then((res) => {
            console.log(res.data)
            if(res.data.state){
              this.form = res.data.data;
              this.form.domesticDestination = res.data.data.domesticDestinationCode;
              setTimeout(() => {
                this.getInitData();
              },100)
            }
        ...
        getInspectAndQuarantine(data){
          this.form.inspectionAndQuarantineName = data.inspectAndQuarantineName;
        },
        //获取检验检疫规格
        inputIgs(){
        ...
        //填入规格型号
        createElements(val){
          this.form.commodityType = val;
        },
        //添加商品
        createCommodity(){
          if(this.isAdd){
              if(this.form.customCommodityNo == '' ){
                this.$message.error('企业商品编号不能为空!')
              }else {
                this.$axios3.post('import/addCommodity',this.form)
                  .then((res) => {
                    // console.log(res.data);
                    if(res.data.state === true){
