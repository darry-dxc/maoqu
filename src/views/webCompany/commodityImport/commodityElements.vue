      ...
      >
        <el-form :model="form" style="height:400px;overflow:hidden;overflow-y:scroll;padding-right:40px">
          ...
          <el-row v-for="item in commodityElements" :key="item.element">
            ...
            <el-select disabled v-else-if="item.element === '出口享惠情况'" v-model="form['出口享惠情况']" filterable :filter-method="preferentialFilter" style="width:100%" size="small">
              <el-option
          ...
          <el-row>
            <el-form-item label="规格型号" :label-width="formLabelWidth" class="form-item">
              <el-input v-model="commodityType" disabled size="small">
    ...
    export default {
      ...
      methods:{
        getItem(val){
          ...
          this.commodityType = inspectGoodsSpecification.join('|')
          this.num = this.commodityType.length;
        },
        createElements(){
          this.$emit('createElements',this.commodityType)
