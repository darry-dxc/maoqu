<template>
	<div>
      ...
      >
				...
				<el-table-column prop="enterprisecustomscode" label="海关代码(进口)" width="180" />
...
<script>
  import moment from 'moment';
	import Add from './../../views/webCompany/deliveryEdit/deliveryAddImport';
  import Edit from './../../views/webCompany/deliveryEdit/deliveryEditImport';
  import Delete from './../../views/webCompany/deliveryEdit/deliveryDeleteImport';
	export default {
	//定义数据
  data () {
    return {
  ...
  methods:{
    //查看收发货单位信息
    viewDelivery(row){
      this.$refs.edit.editDelivery(row.receiveenterpriseid);
    },
    //选中收发货时更改删除按钮的可编辑状态
    ...
    //收发货单位查询
    queryDelivery(){
  	  this.$axios3.post('/imRevEnterprisecusFuzzySearch',{
        ...
        if(res.data.code = 200){
          ...
          this.imReceiveEnterprise = res.data.data.list;
        }
      }).catch((e)=> {
          console.log(e);
      });
