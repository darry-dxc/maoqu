    ...
    <div>
      <el-dialog title="境外收发货人" :visible.sync="dialogFormVisible" width="800px">
        <el-form :model="form">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="所属企业" label-width="150px"  class="form-item">
                <el-input v-model="form.ownedEnterpriseName" size="small" disabled="disabled" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="启运国" label-width="150px" class="form-item" >
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="贸易国(地区)" label-width="150px" class="form-item">
            ...
            <el-col :span="12">
              <el-form-item label="入境口岸" label-width="150px" class="form-item">
                <el-select v-model="form.entrytPort" size="small" style="width: 100%" :filter-method="entrytPortsFilter" filterable clearable >
                  <el-option
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="启运港" label-width="150px" class="form-item">
                <el-select v-model="form.originPort" size="small" style="width: 100%" :filter-method="originPortsFilter" filterable clearable>
                  <el-option
            ...
            <el-col :span="12">
              <el-form-item label="经停港" label-width="150px" class="form-item">
                <el-select v-model="form.stopOverPort" size="small" style="width: 100%" :filter-method="stopOverPortsFilter" filterable clearable >
                  <el-option
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="addOverseasCustomer" size="small">确 定</el-button>
        </div>
      </el-dialog>
    </div>
</template>
  ...
  export default {
    data() {
      return {
        ...
        form: {
          ...
          editor:'',
          lastEditTime:'',
          ownedEnterpriseId:'',//客户所属公司ID
          ownedEnterpriseName:'',//所属公司名称
          customerAddress:'',
          customerPhone:'',//客户电话
          customerFax:'',
    ...
    methods:{
      addOverseasCustomer() {
        if(this.form.customerName === ''){
          ...
          let param = {
            ...
            ownedEnterpriseName: this.form.ownedEnterpriseName,
            customerName: this.form.customerName,
            ovRsCode: this.form.ovRsCode,
            arrivalCountry: this.form.arrivalCountry,
            originPort: this.form.originPort,
            entrytPort: this.form.entrytPort,
          ...
          console.log(param)
          this.$axios3.post('/addOverseasRsCompany', param)
            .then((res) => {
              console.log(res.data);
              if(res.data.code === 200){
                this.$message.success('境外客户添加成功');
                this.dialogFormVisible = false;
                let resData = res.data.data;
                this.customerId=resData.customerId;
                this.$emit('query');
                this.upload();
              }else {
      ...
      stopOverPortsFilter(query){
        if(query !== ''){
          this.stopOverPortsAll = this.ports.filter(item => {
            return (item.value+item.label).toString().includes(query);
          })
        }else {
          this.stopOverPortsAll = this.ports.slice(0,19);
