    ...
    <div style="height: 650px;overflow-y: auto;overflow-x: hidden">
      ...
      <el-form :model="form">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="出境关别" label-width="100px" class="distance">
              <el-select v-model="form.exportBorder" disabled size="small" style="width: 100%;">
                <el-option
                  v-for="item in declareCustomsAll"
                  :key="item.value"
                  :label="item.label"
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="出口日期" label-width="100px" class="distance">
              <el-input v-model="form.exportDate" disabled size="small"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item class="distance">
              <el-input v-model="form.teRsCompanyName" placeholder="企业名称（中文）" disabled auto-complete="off"  size="small"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item class="distance">
              <el-input v-model="form.ovRsCompanyName" placeholder="企业名称（中文）" disabled auto-complete="off"  size="small"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="10">
            <el-form-item label="生产销售单位"  label-width="100px" class="distance">
              <el-input v-model="form.psCreditCode" placeholder="社会信用证代码" disabled auto-complete="off"   size="small"/>
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item class="distance">
              <el-input v-model="form.psCustomsCode" placeholder="海关代码" disabled auto-complete="off"  size="small"/>
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item class="distance">
              <el-input v-model="form.psQuarantineCode" placeholder="10位检验检疫编码" disabled auto-complete="off"  size="small"/>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item class="distance">
              <el-input v-model="form.psCompanyName" placeholder="企业名称（中文）" disabled auto-complete="off"  size="small"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span=6>
            <el-form-item label="运抵国（地区）"  label-width="115px" class="distance">
              <el-select v-model="form.arrivalCountry" disabled size="small" style="width: 100%">
                <el-option
          ...
          <el-col :span="6">
            <el-form-item label="指运港"  label-width="115px" class="distance">
              <el-select v-model="form.arrivalPort" disabled size="small" style="width: 100%">
                <el-option
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="离境口岸"  label-width="115px" class="distance">
              <el-select v-model="form.exportPort" disabled size="small" style="width: 100%;">
                <el-option
        ...
        <el-row>
          <el-col :span="24">
            <el-table :data="goodsData" style="width: 100%" stripe disabled>
              ...
              <el-table-column prop="dealUnit" label="成交单位" :formatter="dealUnitFormat"/>
              ...
              <el-table-column prop="sourceCountry" label="原产国（地区）" :formatter="sourceCountryFormat"/>
            </el-table>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="24">
            <el-table :data="containerData" style="width: 100%" stripe disabled>
              ...
              <el-table-column prop="containerSpecification" label="集装箱规格" />
            ...
            <el-table :data="attachmentData" style="width: 100%" stripe disabled>
              ...
              <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                  ...
                  <el-button size="mini" type="text" :disabled="scope.row.attachmentAddress == undefined" @click="uploadAttach(scope.$index, scope.row)">下载</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-col>
        ...
        <el-dialog title="审核" :visible.sync="dialogresultVisible" width="500px" append-to-body>
          ...
          <el-row>
            <el-col>
              <el-button @click="auditClick" size="small" type="primary" style="float:right; margin:10px;">确 定</el-button>
...
//定义初始化函数
    ...
    methods:{
      ...
      relationshipConChange(){
        if(this.form.relationshipCon === '否'){
          this.form.priceCon = '否';
      ...
      showDocumentInfo(val){
        this.auditOpinion = '';
        console.log('ckckck',val)
        this.id=val.exDeclareDetailId;
        ...
        this.containerData=[];
        let param={
          exDeclareDetailId:val.exDeclareDetailId,
        }
        this.$axios6.post('/declareDetail',param)
          .then((response) => {
        ...
        this.$axios6.get('xml/getExReceipt',{params:{
            exDeclareDetailId:val.exDeclareDetailId
        }})
          .then((res) => {
            console.log('查询',res)
      ...
      accompanyQuery(){
        ...
        if(this.id != undefined){
          let params = {
            'exDeclareDetailId':this.id ,
          };
          console.log('随附单据参数',params)
          this.$axios7.post('export/AccompanyDoc/getbyid',params)
            .then((res) => {
              console.log('随附单据查询',res)
              for(let i = 0;i<res.data.length;i++){
                this.attachmentData.push({
                  exDeclareDetailId:res.data[i].exDeclareDetailId,
      ...
      waitAuditClick(){
        ...
        console.log(param);
        this.$axios10.post('/xml/doExPost',param)
          .then((response) =>{
            ...
            if(response.data.returnCode == 1){
              ...
              console.log(param);
              this.$axios6.post('xml/export/updateInvNoPwd',param)
                .then((response) =>{
      ...
      showDialogHistory () {
        ...
        this.$axios6.get('/getAuditOpinion',{params:params})
          .then((response) => {
            let res = response.data.data;
            if(res != undefined){
              for(let i=0; i<res.length; i++){
                let dataRow = res[i];
                if(this.id == dataRow.exDeclareDetailId){
