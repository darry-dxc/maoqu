    ...
    <div style="height: 800px;overflow: auto;background:white">
      <div v-show="importflag">
        <el-row style="margin: 10px 0;">
          <el-col :span="22">
            ...
            <el-dialog
              ...
              width="60%"
            >
              <el-table :data="unTreatedOrderData" highlight-current-row style="width: 100%;" height="400" @current-change="selectOrder">
                <el-table-column type="index" label="#"/>
                <el-table-column prop="imDeclareServiceId" label="服务单号" width="220" />
                <el-table-column prop="teRsComanyName" label="收发货人" />
            ...
            <el-dialog title="选择历史单据" :visible.sync="dialogOperationVisible" width="750px" >
              <div style="float: left;">
                ...
                <el-button type="primary" size="small" @click="HistoricalDocusFun">查询</el-button>
              </div>
              <div>
                <el-table :data="docuTableData" highlight-current-row @current-change="selectItem" stripe>
                  <el-table-column prop="imDeclareServiceId" label="服务单号" width="180">
                  </el-table-column>
        ...
        <div class="export-left">
          <!--进口信息表单-->
          <el-row >
            <importForm ref="bsForm"/>
          </el-row>
          <el-row >
            <!--商品信息表单-->
            <importCommodityForm ref="commodityForm" v-on:price1="price1" />
          </el-row>
        </div>
...
<script type="text/ecmascript-6">
  import importForm from './businessManage/importBsForm';
  import importCommodityForm from './businessManage/importCommodityForm';
  import container from './businessManage/importContainer';
  import documents from './businessManage/documents';
  import relativeInfo from './businessManage/importRelativeInfo';
  import DeclarationForm from './businessManage/importDeclarationForm';
  import selectHistoricalDocus from './businessManage/importSelectHistoryDocus';
  import AttachmentAndLog from './businessManage/importAttachmentAndLog';
  import attachmentUpload from './businessManage/importAttachmentUpload';
  ...
  import moment from 'moment';
    export default {
      ...
      activated() {
        if(this.$route.query.imDeclareServiceId !== undefined){
          ...
          if(this.$route.query.billStatus == '制单中' || this.$route.query.billStatus == '审核未通过'){
            ...
            this.newOrderInfo(this.$route.query.imDeclareDetailId);
          }
          else if(this.$route.query.billStatus === '2'){
            this.importflag = false;
            this.newOrderFlag=true;
            this.$refs.DecForm.findImportService(this.$route.query.imDeclareServiceId,this.$route.query.businessId);
          }
    ...
    methods:{
      ...
      selectItem(val){
        ...
        if(val !== null && val !== undefined){
          this.$refs.con.isDisabled();
          this.$refs.doc.isDisabled();
          this.newOrderInfo(val.imDeclareDetailId);
          setTimeout(()=>{
      ...
      showAuditOpinion(){
        ...
        // console.log(this.orderInfo.imDeclareDetailId)
        this.$axios8.get('getAuditOpinion',{
          params:{
            'imDeclareDetailId':this.orderInfo.imDeclareDetailId
          }
        })
          .then((res) => {
            console.log(res)
      ...
      containerQuantityChange(data){
        ...
        var containerNum = 0;
        for(let i in data){
          if(data[i].containerSpecfifcaton == 11
            || data[i].containerSpecfifcaton == 12
            || data[i].containerSpecfifcaton == 13
            || data[i].containerSpecfifcaton == 32){
            containerNum += 2;
          }
          else {
            containerNum +=1;
      ...
      orderCon(){
        ...
        //使用人
        this.$store.commit("updateUserInfo",this.orderInfo.userInfoList);
        //所需单证
        ...
        //产品资质
        this.$store.commit("updateProductQualificationList",this.orderInfo.productQualificationList);
        //VIN信息
        this.$store.commit("updateLicenseVINList",this.orderInfo.licenseVINList);
        this.$refs.commodityForm.commodityForm.exDeclareDetailId = this.orderInfo.exDeclareDetailId;
        ...
        //导入随附单证列表
        this.$refs.doc.docData = this.orderInfo.certificateList;
        //随附单证
        var attchDoc = [];
        for (let i in this.orderInfo.certificateList) {
          attchDoc[i] = this.orderInfo.certificateList[i].attachCertificateType;
        }
      ...
      saveOrder(){
        var params = {
          imDeclareDetailId:this.orderInfo.imDeclareDetailId,//进口报关单id
          ...
          enterBorder:this.$refs.bsForm.form.enterBorder,//进境关别
          recordNo:this.$refs.bsForm.form.recordNo,//备案号
          contractNo:this.$refs.bsForm.form.contractNo,//合同协议号
          importDate:this.$refs.bsForm.form.importDate,//进口日期
          ...
          teRsComanyName:this.$refs.bsForm.form.teRsComanyName,//境内收发货人单位名称（中文）
          teRsQuarantineCode:this.$refs.bsForm.form.teRsQuarantineCode,//境内收发货人检验检疫编码
          ovRsCode:this.$refs.bsForm.form.ovRsCode,//境外收发货人代码
          ovRsComanyName:this.$refs.bsForm.form.ovRsComanyName,//境外收发货人单位名称（外文）
          ...
          csCustomsCode:this.$refs.bsForm.form.csCustomsCode,//消费使用单位海关代码
          csCreditCode:this.$refs.bsForm.form.csCreditCode,//消费使用单位信用代码
          csCompanyName:this.$refs.bsForm.form.csCompanyName,//消费使用单位名称
          csQuarantineCode:this.$refs.bsForm.form.csQuarantineCode,//消费使用单位检验检疫编码
          ...
          originPort:this.$refs.bsForm.form.originPort,//启运港
          tradeCountry:this.$refs.bsForm.form.tradeCountry,//贸易国
          originCountry:this.$refs.bsForm.form.originCountry,//启运国
          stopOverPort:this.$refs.bsForm.form.stopOverPort,//经停港
          entryPort:this.$refs.bsForm.form.entryPort,//入境口岸
          ...
          marks:this.$refs.bsForm.form.marks,//备注
          ...
          productQualificationList:this.productQualificationList,//出口报关单关联商品
          licenseVINList:this.licenseVINList,//出口报关单关联商品
        };
        console.log('暂存',params);
        this.$axios8.post('saveDeclareOrder',params)
          .then((res) => {
      ...
      containerListSave(){
        ...
        this.$axios8.post('saveDetailContainer',params)
          .then((res) => {
            // console.log(res.data);
            if(res.data.state === true){
      ...
      attachCertificateListSave(){
        ...
        this.$axios8.post('saveDetailCertificate',params)
          .then((res) => {
            // console.log(res.data);
            if(res.data.state === true){
      ...
      printOrder(){
        ...
        for(var i = 0; i < this.$refs.commodityForm.commodityData.length; i++){
          commodityData.push({
            "commodityName": this.isNull(this.$refs.commodityForm.commodityData[0].commodityName),
            "commoditySpecification": this.isNull(this.$refs.commodityForm.commodityData[0].commoditySpecification),
            "commodityCustomsNo": this.isNull(this.$refs.commodityForm.commodityData[0].commodityCustomsNo),
            "CIQ": '',
            "legalQuantity1": this.isNull(this.$refs.commodityForm.commodityData[0].legalQuantity1),
            "legalMeasurementUnit1": this.dealUnitsFind(this.$refs.commodityForm.commodityData[0].legalMeasurementUnit1),
            "egalQuantity2": this.isNull(this.$refs.commodityForm.commodityData[0].legalQuantity2),
            "legalMeasurementUnit2": this.dealUnitsFind(this.$refs.commodityForm.commodityData[0].legalMeasurementUnit2),
            "dealQuantity": this.isNull(this.$refs.commodityForm.commodityData[0].dealQuantity),
            "dealMeasurementUnit": this.dealUnitsFind(this.$refs.commodityForm.commodityData[0].dealMeasurementUnit),
            "dealUnitPrice": this.isNull(this.$refs.commodityForm.commodityData[0].dealUnitPrice),
            "dealTotalPrice": this.isNull(this.$refs.commodityForm.commodityData[0].dealTotalPrice),
            "currency": this.currencysFind(this.$refs.commodityForm.commodityData[0].currency),
            "originalCountry": this.tradeCountrysFind(this.$refs.commodityForm.commodityData[0].originalCountry),
            "originalCountryCode": this.isNull(this.$refs.commodityForm.commodityData[0].originalCountry),
            "destCountryRegion": this.tradeCountrysFind(this.$refs.commodityForm.commodityData[0].destCountryRegion),
            "domesticDestination": this.destinationcodesFind(this.$refs.commodityForm.commodityData[0].domesticDestination),
            "destCountryRegionCode": this.isNull(this.$refs.commodityForm.commodityData[0].destCountryRegion),
            "levyMode": this.levyModesFind(this.$refs.commodityForm.commodityData[0].levyMode),
            "levyModeCode": this.isNull(this.$refs.commodityForm.commodityData[0].levyMode),
          })
        }
        ...
        console.log('kk',this.$refs.commodityForm.commodityData,commodityData)
        this.$confirm('确定打印此订单?', '提示', {
          ...
          XDoc.run("./进口报关单new.xlsx",
            "pdf",
            {
              "preEntryNo": this.isNull(this.$refs.bsForm.form.preEntryNo),
              "teRsComanyName": this.isNull(this.$refs.bsForm.form.teRsComanyName),
              "pageNum": 1/1 ,
              "enterBorder": this.isNull(this.$refs.bsForm.form.enterBorder),
              "customsNo": this.isNull(this.$refs.bsForm.form.declareCustoms),
              "customsName": this.isNullAndName(this.$refs.bsForm.declareCustomsAll),
              "enterName": this.isNullAndName(this.$refs.bsForm.enterBordersAll),
              "importDate": this.isNull(this.$refs.bsForm.form.importDate),
              "declareDate": this.isNull(this.$refs.bsForm.form.declareDate),
              "recordNo": this.isNull(this.$refs.bsForm.form.recordNo),
              "ovRsComanyName": this.isNull(this.$refs.bsForm.form.ovRsComanyName),
              "transportMode": this.isNull(this.$refs.bsForm.form.transportMode),
              "transportModeName": this.isNullAndName(this.$refs.bsForm.transportModesAll),
              ...
              "storagePlace": this.isNull(this.$refs.bsForm.form.storagePlace),
              "csCompanyName": this.isNull(this.$refs.bsForm.form.csCompanyName),
              "supervisionMode": this.isNull(this.$refs.bsForm.form.supervisionMode),
              "supervisionModeName": this.isNullAndName(this.$refs.bsForm.superviseModesAll),
              "levyNatureName": this.isNullAndName(this.$refs.bsForm.levyNaturesAll),
              "licenseKey": this.isNull(this.$refs.bsForm.form.licenseKey),
              "originPortName": this.isNullAndName(this.$refs.bsForm.originPortsAll),
              "contractNo": this.isNull(this.$refs.bsForm.form.contractNo),
              "tradeCountry": this.isNull(this.$refs.bsForm.form.tradeCountry),
              "tradeCountryName": this.isNullAndName(this.$refs.bsForm.tradeCountrysAll),
              "originCountryName": this.isNullAndName(this.$refs.bsForm.originCountrysAll),
              "stopOverPortName": this.isNullAndName(this.$refs.bsForm.stopOverPortsAll),
              "entryPortName": this.isNullAndName(this.$refs.bsForm.entryPortsAll),
              "packageTypeName": this.isNullAndName(this.$refs.bsForm.packageTypesAll),
              ...
              "incidentalCurrency": this.isNullAndName(this.$refs.bsForm.incidentalCurrencysAll),
              ...
              "originCountry": this.isNull(this.$refs.bsForm.form.originCountry),
              "stopOverPort": this.isNull(this.$refs.bsForm.form.stopOverPort),
              "packageType": this.isNull(this.$refs.bsForm.form.packageType),
              "entryPort": this.isNull(this.$refs.bsForm.form.entryPort),
              "originPort": this.isNull(this.$refs.bsForm.form.originPort),
              ...
              "csCustomsCode": this.isNull(this.$refs.bsForm.form.csCustomsCode)
            },
            "_blank");
        }).catch((e) => {
          console.log(e)
        ...
        if(this.saveOK === true){
          ...
          if(this.$refs.bsForm.submitFormFlag){
            if(this.$refs.bsForm.activeName == 0){
              ...
              this.$axios8.post('submitToVerify',params)
                .then((res) => {
      ...
      clearInfoAll(){
        ...
        this.$refs.bsForm.elsePackagbanding(null);
      ...
      //随附单据上传
      AttachUploadFun(){
        this.$refs.AttachUpload.bussinessIdQuery(this.orderInfo.imDeclareDetailId);
      },
      //显示进口业务界面
      importShow(exDeclareDetailId){
        this.isOrderEdit = false;
        this.importflag = true;
        this.newOrderFlag=false;
        this.$refs.con.isDisabled();
