      ...
      <el-collapse v-model="activeName" @change="activeNameChange" accordion align-center>
        <el-collapse-item name="1">
          <template slot="title">
            ...
            <span style="color: #ff0000; padding-left: 50px">互联网账号存在被盗风险，建议定期更换密码以保护账户安全</span>
            <span style="float: right; font-size: 16px">修改</span>
          </template>
          <div class="stepsDiv">
            <el-steps :active="activePass" finish-status="success">
          ...
          <div class="conDiv">
            ...
            <el-form v-model="passwordVerifyForm" v-if="passwordflag=='A'" label-width="130px" >
              ...
              <el-form-item prop="passCode" label="验证码:">
                ...
                <el-col :span="1">
                  <el-button type="text" style="font-size: 20px;" icon="el-icon-refresh" circle  @click="refreshCode"/>
                </el-col>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="submitVerifyByPass" >提交</el-button>
              </el-form-item>
            </el-form>
            <!-- 通过已验证邮箱身份表单 -->
            <el-form v-model="mailVerifyForm"  v-if="passwordflag=='B'" label-width="120px" >
              ...
              <el-form-item>
                <el-button type="primary" @click="sendVerifyByEmail">发送验证邮件</el-button>
              </el-form-item>
              ...
              <el-form-item>
                <el-button type="primary" @click="submitVerifyByEmail">提交</el-button>
              </el-form-item>
            </el-form>
            <!-- 通过已验证手机身份表单 -->
            <el-form v-model="phoneVerifyForm" v-if="passwordflag=='C'" label-width="150px" >
              ...
              <el-form-item prop="passCode" label="请填写手机验证码:">
                ...
                <el-col :span="5">
                  <el-button type="primary" v-if="show1" @click="sendIdentifyCode1">获取短信校验码</el-button>
                  <el-button id="timer1" type="button" v-if="!show1" disabled>{{count1}} s</el-button>
                </el-col>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="submitVerifyByPhone">提交</el-button>
              </el-form-item>
            </el-form>
            <!-- 设置新密码 -->
            <el-form v-model="newPasswordForm" :rules="rulesNewPass"  label-width="100px" v-if="passwordflag=='D'">
              ...
              <el-form-item>
                <el-button type="primary" @click="saveNewPass">提交</el-button>
              </el-form-item>
            </el-form>
            <!--登录密码修改成功  -->
        ...
        <el-collapse-item name="2">
          <template slot="title">
            ...
            <span style="padding-left: 50px">您验证的邮箱：{{email}}</span>
            <span style="float: right; font-size: 16px">修改</span>
          </template>
          <div class="stepsDiv">
          ...
          <div class="conDiv">
            ...
            <el-form v-model="phoneVerifyForm" v-if="emailflag=='C'" label-width="150px" >
              ...
              <el-form-item prop="passCode" label="请填写手机验证码:">
                ...
                <el-col :span="5">
                  <el-button type="primary" v-if="show2" @click="sendIdentifyCode1">获取短信校验码</el-button>
            ...
            <el-form v-model="newEmailForm" :rules="rulesNewMail" label-width="100px" v-if="emailflag=='D'">
              ...
              <el-form-item>
                <el-button type="primary" @click="saveNewEmail">保存</el-button>
              </el-form-item>
            </el-form>
            <!--发送邮件  -->
            <div class="passSuc" v-if="emailflag=='E'">
              ...
              <el-button type="danger" @click="queryNewEmail">发送验证邮件</el-button>
            </div>
            <!--邮箱验证成功  -->
        ...
        <el-collapse-item name="3">
          <template slot="title">
            ...
            <span style="padding-left: 50px">您验证的手机：{{phoneNumber}}   若已丢失或停用，请立即更换，</span>
          ...
          <div class="conDiv">
            ...
            <el-form v-model="phoneVerifyForm" v-if="phoneflag=='C'" label-width="150px" >
              ...
              <el-form-item prop="passCode" label="请填写手机验证码:">
                ...
                <el-col :span="5">
                  <el-button type="primary" v-if="show3" @click="sendIdentifyCode1">获取短信校验码</el-button>
            ...
            <el-form v-model="newPhoneForm" :rules="rulesNewPhone" label-width="100px" v-if="phoneflag=='D'">
              ...
              <el-form-item label="手机验证码:">
                <el-row>
                  ...
                  <el-col :span="8">
                    <el-button type="primary" @click="sendIdentifyCode" v-if="show">获取短信校验码</el-button>
                    <el-button id="timeri" type="primary" v-if="!show" disabled>{{count}} s</el-button>
                  </el-col>
                </el-row>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="saveNewPhone" >提交</el-button>
              </el-form-item>
            </el-form>
            <!--手机修改成功  -->
...
<script>
  ...
  export default {
    ...
    activated() {
      let dataPersonPhone=JSON.parse(localStorage.getItem('Phone1'));
      this.$store.commit("updatePhone",dataPersonPhone);
      let dataPersonEmail=JSON.parse(localStorage.getItem('Email1'));
      this.$store.commit("updateEmail",dataPersonEmail);
      this.email = this.nickEmail.replace(/(.{2}).+(.{2}@.+)/g, '$1****$2');//隐藏邮箱中间数字
      this.phoneNumber = this.nickPhone.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2');//隐藏手机号中间数字
      this.activeName = '';
      this.activePass = 0;
      this.activeEmail = 0;
      this.activePhone = 0;
    ...
    computed: {
      ...
      nickEmail(){
        return this.$store.state.nickEmail;
      },
    },
    //定义方法
    methods: {
      ...
      submitVerifyByEmail() {
        ...
        console.log(param)
        this.$axios1.post("/emailCodeAuth", param)
          .then((response) => {
          let res = response.data;
          console.log(res);
          if (res.code == 200) {
            this.passTaken = res.data;
            console.log("请求成功",this.activeName);
            if (this.activeName === '1') {
              this.passwordflag = 'D';
              this.activePass = 1;
            }
      ...
      // 保存密码
      saveNewPass() {
        if(this.newPasswordForm.newpassword == this.newPasswordForm.newpasswordCheck){
          this.$axios1.post("/updatePsw", {
            ...
            if (res.code == 200) {
              ...
              var timer = setInterval(function(){
                i--;
                if(i==1){
                  that.$router.push({path:'/WebPortals/Login'});
                }
              },500);
            }
            else {
          ...
          this.$axios1.post("/updateEmail", param)
            .then((response) => {
              ...
              if (res.code == 200) {
                ...
                this.emailflag = 'F';
                this.activeEmail = 3;
                localStorage.setItem('Email1',JSON.stringify(this.newEmailForm.newEmail));
                this.$store.commit("updateEmail",this.newEmailForm.newEmail);
              }
              else {
                this.$message.error(res.msg);
                this.identifyCode = "";
      ...
      saveNewPhone(){
        ...
        this.$axios1.post("/updatePhone", param)
          .then((response) => {
          let res = response.data;
          console.log(res);
          if (res.code == 200) {
            ...
            this.phoneflag = 'E';
            this.activePhone = 3;
            localStorage.setItem('Phone1',JSON.stringify(this.newPhoneForm.newphone));
            this.$store.commit("updatePhone",this.newPhoneForm.newphone);
          }
          else {
            this.$message.error(res.msg);
          }
