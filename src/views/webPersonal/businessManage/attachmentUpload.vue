      ...
      <el-table :data="attachmentTableDataAll" stripe style="width: 100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" type="text" :disabled="scope.row.attachmentAddress == undefined" icon="el-icon-download" @click ="uploadAttach(scope.$index, scope.row)"></el-button>
...
<script>
  import marksAttachment from './components/exportMarksAttachmentUpload'
  export default {
    //定义数据
    data () {
      return {
    ...
    methods:{
      ...
      //标记唛码附件
      showMarksAttachment(){
        this.$refs.marksAttachment.dialogMarksAttachment(this.exDeclareDetailId);
      },
      //根据服务单号查询业务单号
      bussinessIdQuery(exDeclareDetailId){
        console.log(exDeclareDetailId)
      ...
      uploadAttachAndSave(){
        ...
        if(this.attachmentTableData.length == 0){
          if(this.attachment.length == 0){
            ...
            this.$axios7.post('export/AccompanyDoc/insertAccompanyDoc',params)
              .then((res) => {
      ...
      deleteAttachInfo(index,row){
        console.log(this.exDeclareDetailId,row)
        if(row.accompanyDocId != undefined){
          let param = {
            'exDeclareDetailId':this.exDeclareDetailId,
            'accompanyDocType':row.accompanyDocType,
          };
          this.$axios7.post('export/AccompanyDoc/delete',param)
            .then((res) => {
              console.log('删除',res)
          ...
          if(row.accompanyDocNo == undefined){
            ...
            this.fileList = this.attachmentTableData;
          }
      ...
      setCallbackData(){
        ...
        console.log('文件',this.attachmentTableData)
        ...
        for (let i=0;i<this.attachmentTableData.length;i++){
          this.uploadFilePrams.push({
            ...
            "accompanyDocType": this.attachmentTableData[i].accompanyDocType,//当前文件的名称
            "accompanyDocAddress": this.exDeclareDetailId+"/"+this.attachmentTableData[i].documentName,//文件路径，先写死。//"fileSize":
          });
        };
        this.postCallback={
          "callbackUrl":this.callbackUrl+"/export/AccompanyDoc/callback",
          "callbackHost":"oss-cn-hangzhou.aliyuncs.com",
          "callbackBodyType":"application/json",
          "callbackBody":JSON.stringify(this.uploadFilePrams),
        };
        console.log('上传',this.uploadFilePrams)
      ...
      setUpload(){
        if (this.postState === 1){//获取密钥成功
          ...
          console.log("即将上传文件",this.fileList);
