...
<script>
  export default {
    ...
    methods:{
      //进口标记唛码附件
      dialogMarksAttachment(imDeclareDetailId){
        ...
        if(this.imDeclareDetailId != undefined){
          ...
          this.$axios11.post('import/marks/getbyid',params)
            .then((res) => {
      ...
      //删除
      deleteAttach(){
        console.log(this.imDeclareDetailId)
        if('undefined' != typeof this.imDeclareDetailId){
          let param = {
            'imDeclareDetailId':this.imDeclareDetailId,
          };
          this.$axios11.post('import/marks/delete',param)
            .then((res) => {
              console.log('删除',res)
      ...
      setCallbackData(){
        ...
        this.uploadFilePrams.push({
          ...
          "ownerType": '1',
          "markAddress": this.imDeclareDetailId+"/"+this.markName,//文件路径，先写死。//"fileSize":
        });
        this.postCallback={
          "callbackUrl":this.callbackUrl11+"/import/marks/callback",
      ...
      upload(){
        ...
        console.log("子组件中imDeclareDetailId:"+this.imDeclareDetailId);
