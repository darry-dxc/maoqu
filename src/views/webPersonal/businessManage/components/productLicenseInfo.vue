    ...
    methods:{
      ...
      showProLicenDialog(commodityForm){
        ...
        for(let i in this.productQualificationList){
          if(this.productQualificationList[i].imDeclareDetailCommodityId == this.commodityForm.commodityItemNo){
            QualificationList[j] = this.productQualificationList[i];
            j++;
          }
        }
        console.log('产品资质信息',this.productQualificationList,'lll',QualificationList,'lll',this.commodityForm)
        this.TableData = [];
      ...
      saveLicenseInfo(){
        ...
        if(this.productQualificationList != undefined){
          this.productQualificationList.find((item) => {
            if(item.orderNo === this.form.orderNo
              && item.imDeclareDetailCommodityId == this.commodityForm.commodityItemNo){
              isExit = true;
              item.licenseCategoryName = code;
              item.licenseCategory = this.form.licenseCategory;
              item.licenseNo = this.form.licenseNo;
        ...
        if(isExit == false){
          ...
          this.TableData.push({
            ...
            imDeclareDetailProdQualificationId:this.form.orderNo,
            imDeclareDetailCommodityId:this.commodityForm.commodityItemNo,
      ...
      deleteLicenseInfo(index,row){
        ...
        for(var i = 0; i < this.TableData.length;i++){
          ...
          this.TableData[i].imDeclareDetailProdQualificationId = i+1;
        ...
        for(let i in this.licenseVINList){
          if(this.licenseVINList[i].imDeclareDetailCommodityId == row.imDeclareDetailCommodityId
            && this.licenseVINList[i].imDeclareDetailProdQualificationId == row.orderNo){
            VINList[j] = this.licenseVINList[i];
            j++;
          }
          if(this.licenseVINList[i].imDeclareDetailCommodityId == proList[i].imDeclareDetailCommodityId
            && this.licenseVINList[i].imDeclareDetailProdQualificationId > row.orderNo){
            this.licenseVINList[i].imDeclareDetailProdQualificationId = this.licenseVINList[i].imDeclareDetailProdQualificationId - 1;
          }
        }
        if(VINList.length > 0){
        ...
        console.log('资',this.licenseVINList)
        this.$store.commit("updateLicenseVINList",this.licenseVINList);
      },
      //许可证VIN信息
      productLicenseInfo(){
        console.log('VIN',this.licenseVINList)
          ...
          if(typeof this.licenseVINList !== "undefined" && this.licenseVINList.length != 0){
            ...
            for(let i in this.licenseVINList){
              if(this.licenseVINList[i].licenseCategory == this.form.licenseCategory
                ...
                && this.licenseVINList[i].imDeclareDetailCommodityId == this.commodityForm.commodityItemNo
                && this.licenseVINList[i].imDeclareDetailProdQualificationId == this.form.orderNo){
                VINList[j] = this.licenseVINList[i];
                j++;
              }
            }
      ...
      saveLicenseInfoIn(){
        ...
        this.licenseVINList.find((item) => {
          if(item.orderNo == this.formin.orderNo
            && item.imDeclareDetailCommodityId == this.commodityForm.commodityItemNo
            && item.imDeclareDetailProdQualificationId == this.form.orderNo){
            item.vinNo = this.formin.vinNo;
            item.ladingBillOrWayBillDate = this.formin.ladingBillOrWayBillDate;
            item.qualityGuaranteePeriod = this.formin.qualityGuaranteePeriod;
            item.vehicleIdentificationCode = this.formin.vehicleIdentificationCode;
        ...
        if(isExit == false){
          this.TableDataIn.push({
            imDeclareDetailLicenseVinId:this.formin.orderNo,
            imDeclareDetailCommodityId:this.commodityForm.commodityItemNo,
            imDeclareDetailProdQualificationId:this.form.orderNo,
